defprotocol ExWal.Recycler do
  ...
  @spec add(impl :: t(), log_num :: log_num()) :: :ok
...
defmodule ExWal.Recycler.ETS do
  ...
  @spec add(name :: atom(), log_num :: log_num()) :: :ok
  ...
  def handle_add(%__MODULE__{logs: logs, size: size} = state, log_num) do
    state =
      if add_able?(state, log_num) do
        %__MODULE__{
          logs: [log_num | logs],
          size: size + 1,
          max: log_num
