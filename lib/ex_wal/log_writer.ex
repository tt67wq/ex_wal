defmodule ExWal.LogWriter do
  ...
  def start_link({name, fs, log_num, file_path}) do
    GenServer.start_link(__MODULE__, {name, fs, log_num, file_path}, name: name)
  end
  ...
  # ------------ server ------------
  @impl GenServer
  def init({name, fs, log_num, file_path}) do
    {:ok, file} = ExWal.FS.open_read_write(fs, file_path, [])
